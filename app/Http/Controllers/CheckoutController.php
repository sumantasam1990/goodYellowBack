        ...
        try{
            ...
            if($chargeAmount === 0.00) {
                // now we can insert everything order, payment, shipping address
                        $orderNo = 'ord_GY_' . md5(uniqid(). time().$request->user_id.$request->buyer_id);

                        $paymentOrder = new PaymentOrders;

                        $paymentOrder->buyer_id = $request->buyer_id;
                        $paymentOrder->tran_id = 'zero_'.uniqid();
                        $paymentOrder->tran_status = 'zero';
                        $paymentOrder->tran_receipt_url = 'zero';
                        $paymentOrder->tran_amount = 0;
                        ...
                        return response()->json(['succ' => 'zero_payment', 'order' => $orderNo]);
            }
            ...
            return response()->json(['err' => 'Please put your valid card number and expiry month as 2 digits for eg. 04 and the year should be 2 digits for eg. 24. | ' . $th->getMessage() . ' | ' . $th->getLine()]);
        }
